...
- [`experiments/k-address-space/`](experiments/k-address-space/) — pre-registered test of whether cached key vectors behave like content addresses, with RoPE acting as a positional namespace mechanism. **Closed 2026-07-23** (answered negatively; see its `CLOSING-NOTE.md` / `FINDINGS.md`).
...
- [`experiments/wavelet/`](experiments/wavelet/) — whether the KV cache has exploitable multi-scale structure along the sequence axis: can cached entries be collapsed query-agnostically, and does content reordering (free, by attention's permutation invariance) make that possible? W0 pre-registered 2026-07-28.
